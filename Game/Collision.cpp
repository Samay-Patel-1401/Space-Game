...
void Collision::bullet_obsCollision()
{
	for (int i = 0; i < player->bulletList.size(); i++)
	{
		...
		for (int j = 0; j < obs->astroidList.size(); j++)
		{
			...
			{
				player->bulletList[i].onCollision();
				obs->astroidList[j].onCollision();
			}
		}
